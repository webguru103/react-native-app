...
export const PATTERN_PASSWORD: RegExp = /^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.{8,}).*/;
...
export const ConfirmPasswordValidator = (password: string) => (value: string): boolean => {
  return RegExpValidator(PATTERN_PASSWORD, value) && password === value;
}
